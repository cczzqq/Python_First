...
class Course(models.Model):
    ...
    detail = UEditorField(verbose_name=u'课程详情', width=600, height=300, imagePath="courses/%(basename)s_%(datetime)s.%(extname)s", filePath="courses/%(basename)s_%(datetime)s.%(extname)s", default='')
...
# 把课程在后台进行分类显示，分为轮播课程和非轮播课程, adminx.py文件也要对应添加代码
class BannerCourse(Course):
    class Meta:
        verbose_name = u"轮播课程"
        verbose_name_plural = verbose_name
        # 把这个属性设置为True数据库就不会再生成一张表
        proxy = True
